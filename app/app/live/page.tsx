...
import { LiveCalendar } from "@/components/app/LiveCalendar";
import { Radio, LayoutGrid, CalendarDays } from "lucide-react";
...
export default async function LivePage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  const upcoming = lives?.filter((l) => l.status === "scheduled").sort((a, b) => new Date(a.scheduled_at).getTime() - new Date(b.scheduled_at).getTime()) ?? [];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {/* Toggle vue */}
          <div className="flex bg-surface-2 border border-white/[0.07] rounded-xl p-0.5">
            <Link
              href="/app/live"
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                !isCalendar ? "bg-gold/15 text-gold" : "text-muted hover:text-cream"
              }`}
            >
              <LayoutGrid size={13} /> Liste
            </Link>
            <Link
              href="/app/live?view=calendar"
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                isCalendar ? "bg-gold/15 text-gold" : "text-muted hover:text-cream"
              }`}
            >
      ...
      {isCalendar ? (
        <LiveCalendar lives={(lives ?? []).map((l) => ({
          id: l.id,
          title: l.title,
        ...
        <>
          ...
          {live.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {live.map((l) => <LiveCard key={l.id} live={l as never} />)}
              </div>
            </div>
          )}
          ...
          {upcoming.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {upcoming.map((l) => <LiveCard key={l.id} live={l as never} />)}
              </div>
            </div>
          )}
          ...
          {past.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {past.map((l) => <LiveCard key={l.id} live={l as never} />)}
              </div>
            </div>
          )}
